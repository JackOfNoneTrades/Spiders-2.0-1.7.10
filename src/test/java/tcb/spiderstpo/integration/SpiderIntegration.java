...
public class SpiderIntegration {
    ...
    private static void rotation(WorldServer world) throws Exception {
        ...
        for (int degrees = 110; degrees <= 120; degrees++) {
            state.orientationNormal = new tcb.spiderstpo.common.Vec3d(
                Math.sin(Math.toRadians(degrees)),
                Math.cos(Math.toRadians(degrees)),
                0);
            tcb.spiderstpo.common.Vec3d forward = new tcb.spiderstpo.common.Vec3d(0, 0, 1);
            spider.renderYawOffset = state.getOrientation(1)
                .getRotation(forward)
                .getLeft();
            spider.rotationYawHead = spider.renderYawOffset + 30;
            tcb.spiderstpo.common.network.ClimberStateMessage sent = new tcb.spiderstpo.common.network.ClimberStateMessage(
                state);
            io.netty.buffer.ByteBuf bytes = io.netty.buffer.Unpooled.buffer();
            try {
                sent.toBytes(bytes);
                tcb.spiderstpo.common.network.ClimberStateMessage received = new tcb.spiderstpo.common.network.ClimberStateMessage();
